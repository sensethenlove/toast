...
let wrapper;
exports.default = ({ type, items }) => {
    if (!wrapper)
        wrapper = document.getElementById('stl--toast-wrapper');
    function removeToast(toast) {
        if (toast && wrapper) {
            toast.classList.add('hide');
            toast.style.marginBottom = `-${toast.offsetHeight}px`;
            setTimeout(() => {
                wrapper === null || wrapper === void 0 ? void 0 : wrapper.removeChild(toast);
            }, 810);
        }
    }
    ...
    `);
        ...
        const button = toast === null || toast === void 0 ? void 0 : toast.querySelector('button');
        if (toast && button) {
